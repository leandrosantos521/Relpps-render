...
function publicSiteUrl(){ return String(process.env.PUBLIC_SITE_URL || "https://SEU-SITE.onrender.com").replace(/\/$/,""); }
...
async function getProductImageMap(ids){
  ...
  const images={}, missing=[]; const now=Date.now();
  for(const id of unique){ const c=imageCache.get(id); if(c && now-c.savedAt<6*60*60*1000) images[id]=c.urls; else missing.push(id); }
  async function load(id){
    const wait=Math.max(0,350-(Date.now()-lastImageRequestAt)); if(wait) await new Promise(r=>setTimeout(r,wait));
    lastImageRequestAt=Date.now();
    try{ const data=await blingFetch(`/produtos/${encodeURIComponent(id)}`); const detail=data?.data||data||{}; const urls=collectImageValues(detail); imageCache.set(id,{urls,savedAt:Date.now()}); images[id]=urls; }
    catch(e){ console.warn(`[Bling imagem] Produto ${id}:`,e.message); images[id]=[]; }
  }
  let cursor=0; async function worker(){while(cursor<missing.length) await load(missing[cursor++]);}
  await Promise.all(Array.from({length:Math.min(3,missing.length||1)},worker)); return images;
}
